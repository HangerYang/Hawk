...
from .modeling_qwen2_5_kv import Qwen2_5_VLTextModel as Qwen2_5_VLTextModel_debug
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.language_model = Qwen2_5_VLTextModel_debug._from_config(config.text_config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            if input_ids is None:
                image_mask = inputs_embeds == self.model.get_input_embeddings()(
                    torch.tensor(self.config.image_token_id, dtype=torch.long, device=inputs_embeds.device)
                )
                image_mask = image_mask.all(-1)
            else:
        ...
        if pixel_values_videos is not None:
            ...
            if input_ids is None:
                video_mask = inputs_embeds == self.model.get_input_embeddings()(
                    torch.tensor(self.config.video_token_id, dtype=torch.long, device=inputs_embeds.device)
                )
                video_mask = video_mask.all(-1)
            else:
