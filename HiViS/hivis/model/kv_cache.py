...
def initialize_past_key_values(model, max_len=None):
    ...
    # Multimodal wrappers (e.g. Idefics3/SmolVLM) keep the decoder geometry in
    # config.text_config and the decoder blocks under model.model.text_model,
    # so resolve both rather than assuming the LLM-style flat layout.
    config = model.config
    ...
    for _path in (("model", "text_model", "layers"), ("model", "layers"), ("layers",)):
        ...
        if _obj is not None:
            ...
            break
    if _layers is None:
        raise AttributeError("could not locate decoder layers on %s" % type(model).__name__)
    ...
    for i in range(config.num_hidden_layers):
        devices.append(_layers[i].self_attn.q_proj.weight.device)
