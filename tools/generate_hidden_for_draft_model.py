...
class HiddenStateGenerator:
    ...
    def __init__(
        ...
        aux_hidden_states_layer_ids=None,
    ):
        """
        ...
        Args:
            ...
            aux_hidden_states_layer_ids: Target layers whose hidden states are stored
                (the draft config's list); None keeps the wrapper's default layers
        """
        ...
        self.aux_hidden_states_layer_ids = aux_hidden_states_layer_ids
        ...
        try:
            ...
            for k, v in row.items():
                if isinstance(v, torch.Tensor) and v is not None:
                    row[k] = v.to(device)
            if self.aux_hidden_states_layer_ids is not None:
                row["aux_hidden_states_layer_ids"] = list(self.aux_hidden_states_layer_ids)
            results = self.target_model.get_aux_and_target_hiddens(**row)
            # hidden_states: B, N, len(aux_hidden_states_layer_ids)*D
            ...
            # inputs_embeds is ~20% of every .ckpt and no offline trainer reads it:
            # prepare_data_for_draft_model() keeps only input_ids / target_hiddens /
            # hidden_states / attention_mask / loss_mask, and the draft model embeds
            # input_ids itself with the target's embed_tokens.  Storing it costs
            # 1152 B/token of disk and a padded H2D copy per step for nothing.  Set
            # SAVE_INPUTS_EMBEDS=1 if some future consumer needs the tensor itself.
            if os.environ.get("SAVE_INPUTS_EMBEDS", "0") != "1":
                data_point.pop("inputs_embeds", None)
...
def main():
    ...
    try:
        ...
        if args.draft_model_config_path is not None:
            ...
            # Without this the wrapper falls back to its own default layers, and a
            # draft config that names other layers (banded aux mix) would train on
            # hidden states it never asked for.
            aux_hidden_states_layer_ids = getattr(draft_config, "aux_hidden_states_layer_ids", None)
            logger.info(
        ...
        generator = HiddenStateGenerator(
            ...
            aux_hidden_states_layer_ids=aux_hidden_states_layer_ids,
        )
        ...
        # Record WHICH target layers these shards hold. Generation may store
        # more than any one draft consumes, so the offline dataset needs the
        # stored order to pick out the blocks a given draft config asked for.
        if rank == 0 and aux_hidden_states_layer_ids is not None:
            import json as _json

            with open(os.path.join(args.outdir, "aux_layers.json"), "w") as _f:
                _json.dump(
                    {"aux_hidden_states_layer_ids": list(aux_hidden_states_layer_ids)},
                    _f,
                )
