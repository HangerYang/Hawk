...
def parse_args():
    ...
    training_group.add_argument(
        ...
        default=0,
        help=(
            "Dataloader worker processes. Offline training reads one .ckpt per "
            "sample (~7 MB), so with the default 0 that read sits on the "
            "training step's critical path -- measured GPU utilisation was ~3%%. "
            "4-8 hides it behind the draft forward."
        ),
    )
    training_group.add_argument(
        ...
        type=int,
        default=None,
        help="Batches each worker prefetches. Requires --dataloader_num_workers > 0.",
    )
    training_group.add_argument(
...
def train():
    ...
    if init_mix:
        ...
        rank0_print(
            f"banded mix loaded from config freeze={freeze_mix}: "
            f"{draft_model.get_banded_aux_mix_weights()}"
        )
    ...
    dataloader_args = {
        ...
        # Keeping workers alive matters here: they are respawned every epoch
        # otherwise, and each one re-opens the ckpt index.
    ...
    if args.dataloader_prefetch_factor is not None:
        ...
        dataloader_args["dataloader_prefetch_factor"] = args.dataloader_prefetch_factor
    ...
    trainer = Eagle3TrainerFactory.create(
        ...
        # Without this every config-driven loss term -- branch distillation,
        # skew KL, multi-depth CE, the bridge loss, simulated exit
        # probabilities -- reads its weight off a None config and silently
        # trains as if it were switched off. The online entry point has always
        # passed it.
