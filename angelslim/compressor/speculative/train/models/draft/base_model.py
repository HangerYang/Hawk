...
class Eagle3BaseDraftModel(PreTrainedModel, ABC):
    ...
    def build_vocab_mapping(self, dataset, cache_path):
        ...
        if not os.path.exists(cache_path):
            ...
            # Every rank writes this same path, so a plain torch.save lets one
            # rank read another's half-written zip ("failed finding central
            # directory").  Write per-rank, then rename: rename is atomic on the
            # same filesystem, so a reader sees either the old file or a whole
            # new one.
            tmp_path = f"{cache_path}.tmp.{os.getpid()}"
            torch.save(vocab_mapping, tmp_path)
